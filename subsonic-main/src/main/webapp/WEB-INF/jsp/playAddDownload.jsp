...
<c:if test="${param.asTable}"><td></c:if><c:if test="${empty param.playEnabled or param.playEnabled}">
	<c:choose>
		...
		<c:otherwise><a href="javascript:noop()" onclick="top.playlist.onPlay('${path}', 'P');"><img src="<spring:theme code="playImage"/>" alt="<fmt:message key="common.play"/>" title="<fmt:message key="common.play"/>"></a></c:otherwise>
	</c:choose>
</c:if><c:if test="${param.asTable}"></td></c:if>
<c:if test="${param.asTable}"><td></c:if><c:if test="${param.enqueueEnabled and not param.video}"><a href="javascript:noop()" onclick="top.playlist.onPlay('${path}', 'E');"><img src="<spring:theme code="enqueueImage"/>" alt="Enqueue" title="Enqueue"></a></c:if><c:if test="${param.asTable}"></td></c:if>
<c:if test="${param.asTable}"><td></c:if><c:if test="${(empty param.addEnabled or param.addEnabled) and not param.video}"><a href="javascript:noop()" onclick="top.playlist.onPlay('${path}', 'A');"><img src="<spring:theme code="addImage"/>" alt="Add last" title="Add last"></a></c:if><c:if test="${param.asTable}"></td></c:if>
